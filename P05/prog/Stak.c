...
SubTokens *DeleteSub(SubTokens *Ptr) {
    ...
    free(Temp->InputFile);
    if (Temp->Name != NULL) {
        for (int i = 0; i < MasSize(Temp->Name); i++)
            free(Temp->Name[i]);
        free(Temp->Name);
    }
    free(Temp);
    ...
    while (Temp != NULL) {
        ...
        while (Ptr != NULL) {
            if (strcmp(Ptr->Token, "<") == 0) {
                ...
                Ptr = Ptr->Next;
                if (strstr(Ignore, Ptr->Token) == NULL) {
                    Ptr->Prev->InputFile = malloc(strlen(Ptr->Token) + 1);
